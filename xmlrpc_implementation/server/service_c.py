"""
XML-RPC Service C: MergeSort by CGPA
...
class ServiceC:
    """Service C: Sort by CGPA using MergeSort"""
    
    def __init__(self, next_service_url):
        self.next_service_url = next_service_url
        print(f"[Service C] Initialized. Next service: {next_service_url}")
    ...
    def process(self, students_data, accumulated_results):
        ...
        """
        try:
            print(f"[Service C] Received from Service B")
            print(f"[Service C] Processing {len(students_data)} students...")
            ...
            result = MergeSortService.perform_sort(students, "cgpa")
            ...
            accumulated_results['service_c'] = {
                'operation': 'sort_by_cgpa',
                'result': {
            ...
            print(f"[Service C] Sorted {len(sorted_students)} students by CGPA")
            ...
            print(f"[Service C] Error: {str(e)}")
            raise


def main():
    """Start Service C server"""
    host = os.getenv('SERVICE_C_HOST', 'localhost')
    port = int(os.getenv('SERVICE_C_PORT', '8003'))
    service_d_url = os.getenv('SERVICE_D_URL', 'http://localhost:8004')
    ...
    service = ServiceC(service_d_url)
    server.register_instance(service)
    
    print("="*70)
    print(f"Service C (Sort by CGPA) started on {host}:{port}")
    print(f"Next service: {service_d_url}")
    print("="*70)
