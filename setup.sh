...
echo "   - Request-Reply:  cd docker && docker-compose run --rm reqrep-client"
echo "   - MPI:            cd docker && docker-compose run --rm mpi-runner"
echo ""
echo "2. Run performance comparison (includes single vs multi):"
echo "   python performance_test.py"
echo "   Note: Tests gRPC on single machine AND multiple containers"
echo ""
